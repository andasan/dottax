...
const MultipleStudents = ({ batches }: { batches: number[] }) => {
    ...
    <form onSubmit={form.onSubmit((values: any) => console.log(values))}>
      <Stepper active={active} breakpoint="sm">
        <Stepper.Step label="First step" description="Input a batch">
          <StepOne form={form} batches={batches} />
        </Stepper.Step>
  ...
  const StepOne = ({ form, batches }: { form: any, batches: number[] }) => {
    ...
    return (
      <Container p="xl">
        <Title order={3} weight={100} align="center" py={50}>
          Step 1: Select a batch number
        </Title>
        <Box sx={{ maxWidth: 300 }} mx="auto">
          {/* existing */}
          <Select
            data={batches.map(batch => String(batch))}
            label="Batch Number"
            placeholder={batches.length === 0 ? 'No batches available' : 'Select a batch'}
